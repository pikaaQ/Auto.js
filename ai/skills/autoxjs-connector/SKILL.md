...
> **手机端脚本/项目根路径**: `/sdcard/脚本/`  
> `save` 和 `save_project` 推送的脚本和项目都保存在此目录下；`pull_file` 拉取文件和 `exec` 删除脚本也以此路径为基础。所有涉及手机文件路径的操作都以此目录为基准。
...
**推送到手机的测试/验证/探索脚本（即非项目必需、用完即弃的脚本），在工作完成后必须删除。**

这条规则适用于所有通过 `save`/`save_project`/`run`（含 script 内容）推送到手机的临时脚本。判断标准和操作方式：

- **需要保留**（脚本被用户采纳、是项目成果的一部分）→ 不删除，告知用户脚本已保存到手机
- **需要删除**（临时测试、诊断探索、验证功能后无用的脚本）→ 任务完成前**必须**用 `exec` 删除
...
> 此规则与「脚本开发工作流」Step 5 清理一致，是全局性约束，不限于工作流场景。
...
### 推送项目到 App 保存（save_project）
不执行，项目出现在 App 脚本列表中。与 `push_project`（推送并执行）区分使用：
```python
import json, socket, time
payload = json.dumps({"cmd": "save_project", "project_dir": "/root/auto/Auto.js/scripts/MyProject"})
...
### 推送任意文件到手机（利用 save_project）

`save` 只接受文本脚本并强制加 `.js` 后缀，无法推送图片、配置文件等二进制/非脚本文件。

`save_project` 接收 ZIP 包 → App 解压到临时目录 → `saveProject` 复制所有文件到脚本目录（`{scriptDir}/{projectName}/`）。因此可以利用它推送任意文件：

```python
import json, socket, time, tempfile, os, shutil

# 1. 创建临时目录，放入要推送的文件
tmpdir = tempfile.mkdtemp()
with open(os.path.join(tmpdir, 'config.json'), 'w') as f:
    f.write('{"key": "value"}')
shutil.copy('local_image.png', os.path.join(tmpdir, 'image.png'))

# 2. 作为项目推送
payload = json.dumps({"cmd": "save_project", "project_dir": tmpdir})
s = socket.socket(); s.settimeout(30)
s.connect(("127.0.0.1", 19317))
s.sendall((payload + "\n").encode())
time.sleep(3)
